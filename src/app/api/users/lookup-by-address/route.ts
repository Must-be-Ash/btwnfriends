...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.info('🔍 API: Received wallet address lookup request:', address)
    ...
    if (!validation.success) {
      logger.info('❌ API: Invalid wallet address format:', address)
      return NextResponse.json(
        {
    ...
    if (user) {
      logger.info('✅ API: Found user for wallet address:', {
        address,
        userId: user.userId,
        email: user.email,
      ...
      logger.info('❌ API: No user found for wallet address:', address)
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.info('🔍 API: Received POST wallet address lookup:', address)
